...
let private makeCustomReducer
    ...
    fun reducerInput ->
        ...
        match reducerInput.IsClockTick, inputs.Count = custom.InputLabels.Length with
        //| No,  false ->
        ...
        | No, _ | Release, _ ->
            ...
            // propagate state changes posisbly to outputs
            let graph = 
        ...
        | Store state, _ ->
            ...
            let graph = calculateStateChanges graph
            let outputs =
                extractOutputValuesAsMap graph graphOutputs outputLabels
